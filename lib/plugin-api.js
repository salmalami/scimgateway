...
//
scimgateway.postApiUser = async (baseEntity, apiObj, ctx) => {
  const action = 'postApiUser'
  ...
  const [body] = scimgateway.endpointMapper('outbound', apiObj, config.map.user)
...
/*scimgateway.postApiGroup = async (baseEntity, apiObj, ctx) => {
  const action = 'postApiGroup'
  scimgateway.logger.debug(`${pluginName} handling "${action}" apiObj=${JSON.stringify(apiObj)}`)
...
scimgateway.patchApi = async (baseEntity, id, apiObj, ctx) => {
  ...
  const [endpoint, entitlement_value] = id.split('&');
  scimgateway.logger.debug(`${endpoint}[${entitlement_value}]`)

  const method = 'POST'
  const path = `/createrequest`

  const requestor = config.entity.undefined.tokenAuth.username;
  try {
    const body1 = { endpoint: endpoint, entitlement_value: entitlement_value }
    const entitlementResponse = await doRequest(baseEntity, 'POST', '/getEntitlements', body1, ctx);
    const entitlement = entitlementResponse.body.Entitlementdetails[0];
    const entitlementtype = entitlement.entitlementType;
    scimgateway.logger.debug(`${entitlementtype}`)
    const body = {
      requesttype : "ADD",
      username: apiObj.Operations[0].value[0].display,
      endpoint: endpoint,
