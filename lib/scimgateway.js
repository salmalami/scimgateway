...
const Koa = require('koa') //koa: new web framework
const Router = require('koa-router') //router for koa, for defining routes and handling http requests
const bodyParser = require('koa-bodyparser') //for parsing http requests bodies into js objects
const jwt = require('jsonwebtoken') //library for generating & verifying json web tokens
const passport = require('passport') // Authentication middleware for Node.js, used for authenticating users in web applications.
const OIDCBearerStrategy = require('passport-azure-ad').BearerStrategy // Passport strategy for authenticating with Azure Active Directory using the OAuth 2.0 OpenID Connect protocol.
const dot = require('dot-object') //library for manipulating JavaScript objects using dot notation.
const nodemailer = require('nodemailer') //library for sending email messages from node apps
const fs = require('fs') // module to interact with the file system
const path = require('path') // module for working with the file paths
const callsite = require('callsite')
const utils = require('../lib/utils') // custom module
const countries = require('../lib/countries') // custom module
const { createChecker } = require('is-in-subnet') //ip address subnet checking
const { createTerminus } = require('@godaddy/terminus') //graceful shutdown of servers
...
/*  router.delete([`/(|scim/)(!${undefined}|Users|Groups)/:id`,
 ...
 /* router.patch([`/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`,
  ...
  // USER CREATION API
  router.post(['/Users', '/:baseEntity/api'], async (ctx) => {
    logger.debug(`${gwName}[${pluginName}] [POST api]`)
    const contentType = ctx.request.headers['content-type'];
    if (contentType !== 'application/scim+json') {
      const err = new Error('Invalid Content-Type. Only application/scim+json is accepted.');
      ctx.status = 415; // Unsupported Media Type status code
    ...
    if (strBody === '{}' ) {
      ...
      try {
        ...
        if (config.stream.publisher.enabled) {
          ...
          logger.debug(`${gwName}[${pluginName}] publishing "postApiUser" to SCIM Stream and awaiting result`)
          result = await this.publish(streamObj)
        }
        else if ( !apiObj.userName && !apiObj.externalId) {
          ctx.status = 500
          const err = new Error('userName or externalId is mandatory')
          const [e, customErrorCode] = jsonErr(config.scim.version, pluginName, ctx.status, err)
          if (customErrorCode) ctx.status = customErrorCode
          ...
          logger.debug(`${gwName}[${pluginName}] calling "postApiUser" and awaiting result`)
          result = await this.postApiUser(ctx.params.baseEntity, apiObj, ctx.ctxCopy)
        }
        if (result) {
...
// post api GROUP CREATION/ENTITLEMENT
  router.post(['/Groups', '/:baseEntity/api'], async (ctx) => {
    logger.debug(`${gwName}[${pluginName}] [POST api]`)
    const apiObj = ctx.request.body
    const strBody = JSON.stringify(apiObj)
    if (strBody === '{}') {
      ...
      try {
        ...
        if (config.stream.publisher.enabled) {
          ...
          logger.debug(`${gwName}[${pluginName}] publishing "postApiGroup" to SCIM Stream and awaiting result`)
          result = await this.publish(streamObj)
        }
        else if ( !apiObj.displayName && !apiObj.externalId) {
          ctx.status = 500
          const err = new Error('displayName or externalId is mandatory')
          const [e, customErrorCode] = jsonErr(config.scim.version, pluginName, ctx.status, err)
          if (customErrorCode) ctx.status = customErrorCode
          ...
          logger.debug(`${gwName}[${pluginName}] calling "postApiGroup" and awaiting result`)
          result = await this.postApiGroup(ctx.params.baseEntity, apiObj, ctx.ctxCopy)
        }
        if (result) {
          if (typeof result === 'object') result = { result: result }
          else {
  ...
  // {"eventName":"AsignAccessRoleEvent","subjectName":"RACF_System-B","userID":"peter01"}
  //
  router.patch(['/Groups/:id', '/:baseEntity/api/:id'], async (ctx) => {
    const id = ctx?.params?.id
    ...
    if (strBody === '{}') {
      ...
      ctx.status = 500
      ctx.body = apiErr(pluginName, err)
    } else if (!apiObj.schemas || apiObj.schemas[0] !== 'urn:ietf:params:scim:api:messages:2.0:PatchOp') {
      const err = new Error('Invalid request. "schemas" attribute missing or incorrect.');
      ctx.status = 400; // or 422 for semantic error
      ctx.body = apiErr(pluginName, err);
    } else {
      const operations = apiObj.Operations || [];

      // Check if each operation has 'op' attribute and it equals 'add'
      const invalidOpCheck = operations.some(op => !op.op || op.op !== 'add');
      if (invalidOpCheck) {
        const err = new Error('Unsupported operation. Only "add" operation is allowed.');
        ctx.status = 400;
        ctx.body = apiErr(pluginName, err);
        return;
      }

      // Check if each operation has 'op' attribute
      const missingOpCheck = operations.some(op => !op.op);
      if (missingOpCheck) {
        const err = new Error('Operation attribute missing. Each operation must have "op" attribute.');
        ctx.status = 400;
        ctx.body = apiErr(pluginName, err);
        return;
      }
...
const apiErr = (pluginName, err) => {
  ...
  // Check if the error is in a specific format
  if (
      err.statusCode !== undefined &&
      err.statusMessage !== undefined &&
      err.body !== undefined &&
    ...
    try {
      ...
      if (typeof parsedError === 'object' && parsedError.body && parsedError.body.message !== undefined) {
        detail = parsedError.body.message;
      } else {
